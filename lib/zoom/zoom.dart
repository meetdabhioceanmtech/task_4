                ...
                (value) async {
                  if (value != null) {
                    ...
                    if (isDone != null) {
                      ...
                      CustomSnackbar.show(
                        snackbarType: SnackbarType.SUCCESS,
                        message: 'Can\'t save the file? Try again.',
                      );
                    }
                  }
                },
                    ...
                    for (int index = 0; index < positions.length; index++)
                      Positioned(
                        ...
                        child: GestureDetector(
                          ...
                          onScaleStart: (details) {
                            _normalizedOffset = (details.focalPoint - _offset) / imageScales[index];
                            _previousScale = imageScales[index];
                            _initialRotation = rotation[index];
                          },
                          onScaleUpdate: (details) {
                            rotation[index] = _initialRotation + details.rotation;
                            if (details.pointerCount == 2) {
                              setState(() {
                                imageScales[index] = _previousScale * details.scale;
                                _offset = details.focalPoint - (_normalizedOffset * imageScales[index]);
                              });
                            } else {
                              setState(() {
                                positions[index] += details.focalPointDelta;
                                double clampedX = positions[index].dx;
                                double clampedY = positions[index].dy;
                                positions[index] = Offset(clampedX, clampedY);
                              });
                            }
                          },
                          onScaleEnd: (details) {},
                          child: Transform.rotate(
                            ...
                            child: Transform.scale(
                              ...
                              child: SizedBox(
                                width: MediaQuery.of(context).size.width * 0.8 * imageScales[index],
                                child: Image.file(
                                  File(images[index]),
                                  fit: BoxFit.contain,
                                ),
  ...
  Future<void> pickImages() async {
    ...
    if (pickedImages.isNotEmpty) {
      ...
      for (XFile image in pickedImages) {
        images.add(File(image.path).path);
        imageScales.add(1.0);
        rotation.add(0.0);
